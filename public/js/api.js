...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateUserElements() {
        ...
        document.querySelectorAll('.account-link').forEach(link => {
            if (isLoggedIn) {
                link.href = '/public/html/account/my-account.html';
                link.textContent = `Olá, ${userData.name || 'Usuário'}`;
            } else {
                link.href = '/public/html/login.html';
    ...
    function checkPageProtection() {
        ...
        if (document.body.hasAttribute('data-auth-required')) {
            if (!window.isAuthenticated()) {
                window.location.href = '/public/html/login.html?redirect=' + encodeURIComponent(window.location.pathname);
                return;
            }
        }
