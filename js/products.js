document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCheckboxesFromFilters() {
    ...
    setTimeout(() => {
      ...
      filters.objectives.forEach((objectiveId) => {
        const checkbox = document.querySelector(
          `input[name="objective"][value="${objectiveId}"]`
        );
        if (checkbox) checkbox.checked = true;
      });
      ...
      filters.accessories.forEach((accessoryId) => {
        const checkbox = document.querySelector(
          `input[name="accessory"][value="${accessoryId}"]`
        );
        if (checkbox) checkbox.checked = true;
      });
  ...
  function updateFilterOptions(filtersData) {
    const categoriesContainer = document.querySelector(
      '.filter-group[data-filter="categories"] .filter-options'
    );
    const flavorsContainer = document.querySelector(
      '.filter-group[data-filter="sabores"] .filter-options'
    );
    const brandsContainer = document.querySelector(
      '.filter-group[data-filter="marcas"] .filter-options'
    );
    const objectivesContainer = document.querySelector(
      '.filter-group[data-filter="objectives"] .filter-options'
    );
    const accessoriesContainer = document.querySelector(
      '.filter-group[data-filter="accessories"] .filter-options'
    );

    // Atualizar preços mínimo e máximo
    if (
      filtersData.minPrice !== undefined &&
      filtersData.maxPrice !== undefined
    ) {
      if (minPriceInput)
        minPriceInput.placeholder = `R$ ${filtersData.minPrice
          .toFixed(2)
          .replace(".", ",")}`;
      if (maxPriceInput)
        maxPriceInput.placeholder = `R$ ${filtersData.maxPrice
          .toFixed(2)
          .replace(".", ",")}`;
    }
  ...
  function closeMobileFilters() {
    ...
    if (filtersSidebar && filtersSidebar.classList.contains("mobile-active")) {
      ...
      filtersSidebarOriginalParent.insertBefore(
        filtersSidebar,
        filtersSidebarOriginalNextSibling
      );
    }
  ...
  function closeMobileSort() {
    ...
    if (sortOptions && sortOptions.classList.contains("mobile-active")) {
      ...
      sortOptionsOriginalParent.insertBefore(
        sortOptions,
        sortOptionsOriginalNextSibling
      );
    }
          ...
          if (this.checked) {
            ...
            switch (filterType) {
              case "category":
                ...
                break;
              case "flavor":
                filters.flavors = filters.flavors.filter(
                  (item) => item !== filterValue
                );
                break;
              case "brand":
                ...
                break;
              case "objective":
                filters.objectives = filters.objectives.filter(
                  (item) => item !== filterValue
                );
                break;
              case "accessory":
                filters.accessories = filters.accessories.filter(
                  (item) => item !== filterValue
                );
                break;
            }
  ...
  function updateProductsDisplay(items) {
    ...
    items.forEach((product) => {
      const brandName =
        product.attributes.find((attr) => attr.brand)?.brand ||
        "Marca Desconhecida";
      ...
      <img src="${product.imageUrl || "/assets/images/placeholder.png"}" alt="${
        product.name
      }" class="w-full h-48 object-cover" style="cursor:pointer; background:#fff;">
    </a>
                    ${
                      product.isNew
                        ? '<span class="absolute top-2 left-2 bg-green-500 text-white text-xs font-semibold px-2 py-1 rounded">NOVO</span>'
                        : ""
                    }
                    ${
                      product.discountPercentage
                        ? `<span class="absolute top-2 right-2 bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded">-${product.discountPercentage}%</span>`
                        : ""
                    }
                ...
                <div class="p-4">
                    <h3 class="product-title text-lg font-semibold mb-1 truncate">${
                      product.name
                    }</h3>
                    <p class="text-sm text-white-500">${brandName}</p>                    
                    <div class="flex justify-between items-center">
                        <span class="product-price text-xl font-bold text-orange-500">R$ ${product.price
                          .toFixed(2)
                          .replace(".", ",")}</span>
                        ${
                          product.oldPrice
                            ? `<span class="text-sm text-gray-400 line-through">R$ ${product.oldPrice
                                .toFixed(2)
                                .replace(".", ",")}</span>`
                            : ""
                        }
                    ...
                    <a href="/product-detail.html?id=${
                      product.id
                    }" class="add-to-cart mt-3 w-full bg-orange-500 text-white py-2 rounded hover:bg-orange-600 transition-colors duration-300" data-product-id="${
        product.id
      }">
                        COMPRAR
                    </a>
