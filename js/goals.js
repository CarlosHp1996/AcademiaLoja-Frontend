...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (exploreButton && goalsSection) {
    exploreButton.addEventListener("click", (e) => {
      ...
      const sectionTop =
        goalsSection.getBoundingClientRect().top + window.scrollY;
...
//const API_BASE_URL = "https://localhost:4242https://academialoja-production.up.railway.app/api";
  ...
  goalCards.forEach((card) => {
    ...
    if (viewProductsBtn && objectiveId) {
      viewProductsBtn.addEventListener("click", function (e) {
        ...
        const url = `/products.html?ObjectiveIds=${objectiveId}&title=${encodeURIComponent(
          objectiveName
        )}`;
