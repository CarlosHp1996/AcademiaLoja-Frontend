...
const API_BASE_URL_AUTH = "https://academialoja-production.up.railway.app/api"; // Ajuste se necessário
...
async function updateCartItemCount() {
  if (!window.authService || !window.authService.isAuthenticated()) {
    console.log(
      "Usuário não autenticado, não atualizando contagem do carrinho."
    );
  ...
  try {
    ...
    if (
      result.hasSuccess &&
      result.value &&
      Array.isArray(result.value.orders)
    ) {
    ...
    updateCartBadgeDisplay(0);
    if (window.authNotifications) {
      window.authNotifications.showNotification(
        `Erro ao atualizar carrinho: ${error.message}`,
        "error"
      );
    }
  }
