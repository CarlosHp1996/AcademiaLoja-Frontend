...
class CartService {
  ...
  async migrateSessionCart() {
    try {
      ...
      const sessionIdGuid = this.sessionId
        ? this.sessionId.replace("session_", "")
        : null;
      if (!sessionIdGuid) {
        console.error(
          "Não foi possível extrair o GUID da sessão para migração."
        );
        return;
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.hasSuccess && result.value) {
          ...
          if (
            result.value.userId &&
            result.value.userId.startsWith("session_")
          ) {
            if (this.sessionId !== result.value.userId) {
              this.sessionId = result.value.userId;
              localStorage.setItem("sessionId", this.sessionId);
              console.log(
                "Session ID sincronizado com o backend:",
                this.sessionId
              );
            }
          }
  ...
  async addItem(
    ...
    try {
      ...
      if (response.ok && result.hasSuccess) {
        ...
        const errorMessage =
          result.errors?.[0] || "Erro ao adicionar produto ao carrinho";
  ...
  async updateItemQuantity(productId, quantity) {
    try {
      ...
      const response = await fetch(
        `${this.API_BASE_URL}/Cart/update/${productId}`,
        {
          method: "PUT",
          headers: this.getHeaders(),
      ...
      if (response.ok && result.hasSuccess) {
        ...
        const errorMessage =
          result.errors?.[0] || "Erro ao atualizar quantidade";
        this.showErrorMessage(errorMessage);
        return { success: false, error: errorMessage };
      }
    } catch (error) {
  ...
  async removeItem(productId) {
    try {
      ...
      const response = await fetch(
        `${this.API_BASE_URL}/Cart/remove/${productId}`,
        {
          method: "DELETE",
          headers: this.getHeaders(),
          credentials: "include",
  ...
  handleCheckout() {
    ...
    if (this.isUserAuthenticated()) {
      ...
      if (window.authNotifications) {
        window.authNotifications.loginRequired(
  ...
  updateCartBadge() {
    ...
    if (badge && this.cart) {
      ...
      if (this.cart.totalItems !== undefined) {
        ...
        totalItems = this.cart.items.reduce(
  ...
  createCartItemElement(item) {
    ...
    const variantText = [];
    if (item.flavor && item.flavor !== "Sem sabor")
      variantText.push(`Sabor: ${item.flavor}`);
    if (item.size && item.size !== "Padrão")
      variantText.push(`Tamanho: ${item.size}`);
    ...
    itemDiv.innerHTML = `
                ...
                <div class="cart-item-details">
                    ...
                    ${
                      variantText.length > 0
                        ? `<div class="cart-item-variant">${variantText.join(
                            " • "
                          )}</div>`
                        : ""
                    }
            ...
            <div class="cart-item-controls">
                <div class="quantity-controls">
                    <button class="quantity-btn minus" data-action="decrease" data-product-id="${
                      item.productId
                    }">
                        <i class="fas fa-minus"></i>
                    </button>
                    <span class="quantity-display">${item.quantity}</span>
                    <button class="quantity-btn plus" data-action="increase" data-product-id="${
                      item.productId
                    }">
                        <i class="fas fa-plus"></i>
                    </button>
                </div>
                <button class="remove-item-btn" data-action="remove" data-product-id="${
                  item.productId
                }">
  ...
  hideLoadingState() {
    ...
    if (addToCartBtn) {
      ...
      addToCartBtn.innerHTML =
        '<span class="btn-text">COMPRAR</span><i class="fas fa-shopping-cart btn-icon"></i>';
    }
  }
