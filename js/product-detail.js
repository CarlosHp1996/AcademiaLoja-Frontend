document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderProduct(product) {
    ...
    if (product.oldPrice && product.oldPrice > product.price) {
      priceContainer.innerHTML = `
                <span class="old-price">De: R$ ${product.oldPrice.toFixed(
                  2
                )}</span>
                <span class="current-price">Por: R$ ${product.price.toFixed(
                  2
                )}</span>
            `;
    } else {
      priceContainer.innerHTML = `<span class="current-price">R$ ${product.price.toFixed(
        2
      )}</span>`;
    }
  ...
  function renderFlavorDisplay(attributes) {
    ...
    flavorDisplay.textContent = productFlavorString
      .replace(/([A-Z])/g, " $1")
      .trim();
  ...
  function setupAddToCart() {
    ...
    addToCartBtn.addEventListener("click", async () => {
      ...
      if (
        produto.attributes &&
        produto.attributes.length > 0 &&
        produto.attributes[0].flavor
      ) {
  ...
  async function fetchRelatedProducts() {
    const categoria =
      produto.attributes && produto.attributes[0]
        ? produto.attributes[0].category
        : null;
    ...
    try {
      ...
      if (data && data.hasSuccess && data.value && data.value.products) {
        ...
        const filteredProducts = data.value.products.filter(
          (p) => p.id !== produto.id
        );
    ...
    fetchRelatedProducts()
      .then((relatedProducts) => {
        ...
        if (!relatedProductsContainer) {
          console.error(
            "Elemento 'related-products-container' não encontrado no DOM"
          );
          return;
        }
      ...
      .catch((error) => {
        ...
        if (relatedProductsContainer) {
          relatedProductsContainer.innerHTML =
            "<p>Erro ao carregar produtos relacionados.</p>";
        }
      });
  }
