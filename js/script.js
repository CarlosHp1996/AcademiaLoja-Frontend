document.addEventListener("DOMContentLoaded", () => {
  ...
  const categories = [
    ...
    },
    {
      ...
      enumValue: 3,
      href:
        window.NavigationEnums?.generateFilterUrl("category", "Creatina") ||
        "/products.html?CategoryIds=3",
  ...
  function renderCategories() {
    ...
    categories.forEach((category, index) => {
      ...
      categoryCard.addEventListener("click", (e) => {
        ...
        // Log para debug
        console.log(
          `Navegando para categoria: ${category.name} (ID: ${category.enumValue})`
        );
      });
  ...
  async function renderFeaturedProducts() {
    ...
    try {
      ...
      products.forEach((product, index) => {
        ...
        const reviews =
          product.reviewCount || Math.floor(Math.random() * 200) + 10; // Reviews aleatórios se não vier da API
  ...
  <div class="category-image-container">
    <img src="${imageUrl}" alt="${
          product.name
        }" class="category-image" loading="lazy" onerror="this.src='/placeholder.svg?height=400&width=400'">
    <div class="category-overlay"></div>
    ${badge ? `<div class="product-badge-modern">${badge}</div>` : ""}
    ${
      discountPercentage > 0
        ? `<div class="product-discount-modern">-${discountPercentage}%</div>`
        : ""
    }
  </div>
  <div class="category-content-modern">
    ...
    <div class="product-price-modern">
      ${
        hasDiscount
          ? `<span class="product-old-price-modern">R$ ${product.oldPrice
              .toFixed(2)
              .replace(".", ",")}</span>`
          : ""
      }
      <span class="product-current-price-modern">R$ ${product.price
        .toFixed(2)
        .replace(".", ",")}</span>
    </div>
    <div class="category-cta">
...
`;
        ...
        if (ctaElement) {
          ctaElement.addEventListener("click", (e) => {
            ...
            ctaElement.innerHTML =
              '<i class="fas fa-spinner fa-spin"></i> <span>Adicionando...</span>';

            setTimeout(() => {
              ctaElement.innerHTML =
                '<i class="fas fa-check"></i> <span>Adicionado!</span>';
      ...
      console.log(
        `${products.length} produtos em destaque renderizados com sucesso!`
      );
    } catch (error) {
  ...
  async function fetchProductsByCategory(categoryId, categoryKey) {
    try {
      const apiUrl = `https://academialoja-production.up.railway.app/api/Product/get?CategoryIds=${categoryId}`;

      console.log(
        `Buscando produtos para categoria: ${categoryKey} (ID: ${categoryId})`
      );
      ...
      console.error(
        `Erro ao buscar produtos da categoria ${categoryKey}:`,
        error
      );
      return [];
    }
  ...
  async function initializeCategoryCounters() {
    ...
    for (const card of categoryCards) {
      ...
      try {
        ...
        console.error(
          `Erro ao inicializar contador para categoria ${categoryKey}:`,
          error
        );
      }
    }
  ...
  document
    .querySelectorAll(".category-card-modern, .product-card")
    .forEach((el) => {
      observer.observe(el);
    });
...
document
  .getElementById("btn-ver-destaques")
...
document
  .querySelector(".btn-primary-modern")
  .addEventListener("click", function (e) {
    e.preventDefault();
