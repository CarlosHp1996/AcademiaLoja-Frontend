document.addEventListener("DOMContentLoaded", function () {
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          "https://academialoja-production.up.railway.app/api/Order/create",
          "POST",
          orderData
        );
        ...
        if (response && response.hasSuccess) {
          ...
          window.showNotification(
            "Erro ao adicionar pedido: " +
              (response?.errors?.join(", ") || "Erro desconhecido"),
            "error"
          );
        }
      } catch (error) {
  ...
  document
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Order/update/${orderId}`,
          "PUT",
          orderData
        );
  ...
  document
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Order/delete/${orderId}`,
          "DELETE"
        );
  ...
  async function loadOrders(page, size) {
    ...
    let endpoint = `https://academialoja-production.up.railway.app/api/Order/get?pageNumber=${page}&pageSize=${size}`;
    ...
    try {
      ...
      if (response && response.value && response.value.items) {
        ...
        response.value.items.forEach((order) => {
          ...
          const paymentClass =
            paymentStatus === "Paid" ? "status-completed" : "status-pending";
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-action btn-view" title="Ver detalhes" onclick="viewOrder('${
                                      order.id
                                    }')">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <button class="btn-action btn-edit" title="Editar" onclick="editOrder('${
                                      order.id
                                    }')">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn-action btn-delete" title="Excluir" onclick="deleteOrder('${
                                      order.id
                                    }')">
        ...
        tableBody.innerHTML =
          '<tr><td colspan="7" class="text-center">Erro ao carregar pedidos</td></tr>';
      }
    } catch (error) {
  ...
  async function loadUsers() {
    try {
      const response = await window.apiCall(
        "https://academialoja-production.up.railway.app/api/Auth/get?pageSize=100"
      );

      const userSelects = document.querySelectorAll(
        "#orderUserId, #editOrderUserId"
      );
  ...
  async function loadProducts() {
    try {
      const response = await window.apiCall(
        "https://academialoja-production.up.railway.app/api/Product/get?pageSize=100"
      );
      ...
      if (response && response.value && response.value.items) {
        ...
        response.value.items.forEach((product) => {
          window.productOptions += `<option value="${product.id}">${
            product.name
          } - ${window.formatCurrency(product.price)}</option>`;
        });
    ...
    newItem
      ...
      .addEventListener("click", function () {
        ...
        if (remainingItems.length === 1) {
          remainingItems[0].querySelector(".remove-item").style.display =
            "none";
        }
...
async function editOrder(id) {
  try {
    const response = await window.apiCall(
      `https://academialoja-production.up.railway.app/api/Order/get/${id}`
    );
    ...
    if (response && response.hasSuccess && response.value) {
      ...
      document.getElementById("editOrderShippingAddress").value =
        order.shippingAddress || "";
      document.getElementById("editOrderShippingMethod").value =
        ...
        order.items.forEach((item, index) => {
          ...
          itemElement
            ...
            .addEventListener("click", async function () {
              ...
              if (itemId) {
                try {
                  const deleteResponse = await window.apiCall(
                    `https://academialoja-production.up.railway.app/api/Order/delete/${order.id}/item/${itemId}`,
                    "DELETE"
                  );
