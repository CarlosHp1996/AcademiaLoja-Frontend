document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          "https://academialoja-production.up.railway.app/api/Auth/update",
          "PUT",
          userData
        );
  ...
  document
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Auth/delete/${userId}`,
          "DELETE"
        );
  ...
  async function loadUsers(page, size) {
    ...
    let endpoint = `https://academialoja-production.up.railway.app/api/Auth/get?pageNumber=${page}&pageSize=${size}`;
    ...
    try {
      ...
      if (response && response.value && response.value.items) {
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-action btn-view" title="Ver detalhes" onclick="viewUser('${
                                      user.id
                                    }')">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <button class="btn-action btn-edit" title="Editar" onclick="editUser('${
                                      user.id
                                    }')">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn-action btn-delete" title="Excluir" onclick="deleteUser('${
                                      user.id
                                    }', '${user.name}')">
        ...
        tableBody.innerHTML =
          '<tr><td colspan="7" class="text-center">Erro ao carregar usuários</td></tr>';
      }
    } catch (error) {
...
async function editUser(id) {
  try {
    const response = await window.apiCall(
      `https://academialoja-production.up.railway.app/api/Auth/get/${id}`
    );
