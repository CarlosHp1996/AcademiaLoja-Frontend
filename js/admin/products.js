document.addEventListener("DOMContentLoaded", function () {
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          "https://academialoja-production.up.railway.app/api/Product/create",
          "POST",
          productData
        );
  ...
  document
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Product/update/${productId}`,
          "PUT",
          productData
        );
  ...
  document
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Product/delete/${productId}`,
          "DELETE"
        );
  ...
  async function loadProducts(page, size) {
    ...
    let endpoint = `https://academialoja-production.up.railway.app/api/Product/get?pageNumber=${page}&pageSize=${size}`;
      ...
      if (response && response.value && response.value.items) {
        ...
        response.value.items.forEach((product) => {
          ...
          html += `
            ...
            product.name
          }" width="50" height="50" style="object-fit: cover; border-radius: 4px; cursor:pointer;">
                                </a>
                            </td>
                            <td>${product.name}</td>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-action btn-view" title="Ver detalhes" onclick="viewProduct('${
                                      product.id
                                    }')">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                    <button class="btn-action btn-edit" title="Editar" onclick="editProduct('${
                                      product.id
                                    }')">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn-action btn-delete" title="Excluir" onclick="deleteProduct('${
                                      product.id
                                    }', '${product.name}')">
        ...
        tableBody.innerHTML =
          '<tr><td colspan="8" class="text-center">Erro ao carregar produtos</td></tr>';
      }
    } catch (error) {
...
async function editProduct(id) {
  try {
    const response = await window.apiCall(
      `https://academialoja-production.up.railway.app/api/Product/get/${id}`
    );
