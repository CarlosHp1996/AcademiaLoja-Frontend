document.addEventListener("DOMContentLoaded", function () {
  ...
  // Create payment
  document
    .getElementById("createPaymentBtn")
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Payment/create/${orderId}`,
          "POST"
        );

        if (response && response.hasSuccess) {
          window.showNotification(
            "Pagamento criado com sucesso! Redirecionando para checkout...",
            "success"
          );
  ...
  // Refund payment
  document
    .getElementById("refundPaymentBtn")
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Payment/refund/${paymentId}`,
          "POST",
          refundData
        );
  ...
  document
    .getElementById("detailRefundBtn")
    .addEventListener("click", function () {
  ...
  async function loadPayments(page, size) {
    ...
    try {
      ...
      const response = await window.apiCall(
        `https://academialoja-production.up.railway.app/api/Payment/verify/all?pageNumber=${page}&pageSize=${size}`
      );
      ...
      if (response && response.value && response.value.items) {
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-action btn-view" title="Ver detalhes" onclick="viewPayment('${
                                      payment.id
                                    }')">
        ...
        tableBody.innerHTML =
          '<tr><td colspan="8" class="text-center">Erro ao carregar pagamentos</td></tr>';
      }
    } catch (error) {
  ...
  async function loadOrders() {
    try {
      const response = await window.apiCall(
        "https://academialoja-production.up.railway.app/api/Order/get?pageSize=100&status=Pending"
      );
      ...
      if (response && response.value && response.value.items) {
        ...
        response.value.items.forEach((order) => {
          options += `<option value="${order.id}">${order.id.substring(
            0,
            8
          )}... - ${order.userName} - ${window.formatCurrency(
            order.total
          )}</option>`;
        });
...
async function viewPayment(id) {
  try {
    const response = await window.apiCall(
      `https://academialoja-production.up.railway.app/api/Payment/verify/${id}`
    );
    ...
    if (response && response.hasSuccess && response.value) {
      ...
      document.getElementById("detailDate").textContent = window.formatDate(
        payment.createdAt
      );
      document.getElementById("detailTransactionId").textContent =
        payment.transactionId || "N/A";
      document.getElementById("detailLastFour").textContent =
        payment.lastFour || "N/A";
