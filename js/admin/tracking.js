document.addEventListener("DOMContentLoaded", function () {
    ...
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          "https://academialoja-production.up.railway.app/api/Tracking",
          "POST",
          trackingData
        );
  ...
  // Update tracking
  document
    .getElementById("updateTrackingBtn")
    .addEventListener("click", async function () {
      ...
      try {
        const response = await window.apiCall(
          `https://academialoja-production.up.railway.app/api/Tracking/${trackingId}`,
          "PUT",
          trackingData
        );
  ...
  async function loadTrackings(page, size) {
    ...
    let endpoint = `https://academialoja-production.up.railway.app/api/Tracking?pageNumber=${page}&pageSize=${size}`;
    ...
    try {
      ...
      if (response && response.value && response.value.items) {
        ...
        if (response.value.items.length === 0) {
          tableBody.innerHTML =
            '<tr><td colspan="7" class="text-center">Nenhum rastreio encontrado</td></tr>';
          return;
        }
        ...
        response.value.items.forEach((tracking) => {
          ...
          html += `
                        <tr>
                            ...
                            <td>${window.formatDate(
                              tracking.updatedAt || tracking.createdAt
                            )}</td>
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-action btn-edit" title="Editar" onclick="editTracking('${
                                      tracking.id
                                    }')">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn-action btn-delete" title="Excluir" onclick="deleteTracking('${
                                      tracking.id
        ...
        tableBody.innerHTML =
          '<tr><td colspan="7" class="text-center">Erro ao carregar rastreios</td></tr>';
      }
    } catch (error) {
  ...
  async function loadOrders() {
    try {
      const response = await window.apiCall(
        "https://academialoja-production.up.railway.app/api/Order/get?pageSize=100&status=Processing,Shipped"
      );

      const orderSelects = document.querySelectorAll(
        "#trackingOrderId, #editTrackingOrderId"
      );
...
async function editTracking(id) {
  try {
    const response = await window.apiCall(
      `https://academialoja-production.up.railway.app/api/Tracking/${id}`
    );
