...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadOrderDetails() {
    ...
    try {
      ...
      if (result.hasSuccess && result.value) {
        ...
        throw new Error(
          result.errors?.[0] || "Erro ao carregar detalhes do pedido"
        );
      }
    } catch (error) {
      console.error("Erro ao carregar detalhes do pedido:", error);
      showError(
        error.message || "Erro ao carregar detalhes do pedido. Tente novamente."
      );
    } finally {
      hideLoading();
    }
  ...
  function renderOrderDetails(order) {
    ...
    orderTitle.textContent = `Detalhes do Pedido #${
      order.orderNumber || order.id.substring(0, 8)
    }`;
    orderSubtitle.textContent = `Realizado em ${formatDate(
    ...
    orderStatusInfo.innerHTML = `
        <div class="status-card ${statusClass}">
            ...
            <div class="status-content">
                <h3>${statusText}</h3>
                <p>Última atualização: ${formatDateTime(
                  order.updatedAt || order.orderDate
                )}</p>
                ${
                  order.paymentStatus
                    ? `<p class="payment-status">Pagamento: ${getPaymentStatusText(
                        order.paymentStatus
                      )}</p>`
                    : ""
                }
  ...
  function renderDeliveryInfo(order) {
    ...
    const address =
      Array.isArray(order.addresses) && order.addresses.length > 0
        ? order.addresses[0]
        : null;
        ...
        <div class="delivery-card">
          ...
          <div class="info-group">
            ...
            <div class="shipping-info">
              ...
              ${
                order.trackingCode
                  ? `<p><strong>Código de Rastreamento:</strong> <span class="tracking-code">${order.trackingCode}</span></p>`
                  : ""
              }
            </div>
          </div>
  ...
  function renderProductsList(items) {
    if (!items || items.length === 0) {
      productsList.innerHTML =
        '<div class="no-items">Nenhum produto encontrado neste pedido.</div>';
      return;
    }
        ...
        <div class="payment-card">
            ...
            <div class="info-group">
                ...
                <div class="payment-status-info">
                    <p class="status ${getPaymentStatusClass(
                      order.paymentStatus
                    )}">
